...
namespace Vision.Tools
{
    ...
    public partial class UcCenterDetectTool:UserControl
    {
        ...
        private void UcDetectTool_Load(object sender,System.EventArgs e)
        {
            GetImageIn();
            //显示模板点
            labelModel.Text = $"模板点:      X:{_station.DataConfig.CalibConfig.ModelOriginPoint.X}" +
        ...
        private void btnModel_Click(object sender,EventArgs e)
        {
            var dialog = MessageBox.Show("是否确定更新模板点？\r\n这将保存此工具的最后结果！",
                "重要提示",MessageBoxButtons.YesNo,MessageBoxIcon.Question);
            if(dialog == DialogResult.Yes)
            {
                try
                {
                    if(_tool.ToolBlock.Outputs["X"].Value != null &&
                           _tool.ToolBlock.Outputs["Y"].Value != null &&
                           _tool.ToolBlock.Outputs["Angle"].Value != null)
                    {
                        _station.DataConfig.CalibConfig.ModelOriginPoint =
                            new PointA(
